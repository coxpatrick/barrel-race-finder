...
export const getMonthNumber = (dateStr: string): number =>
  new Date(dateStr + 'T00:00:00').getMonth() + 1
...
export const filterEvents = (events: BarrelRace[], filters: EventFilters): BarrelRace[] => {
  return events.filter(event => {
    ...
    // ── Search: match every space-separated token against name/city/state/arena
    ...
    // ── Month
    if (filters.month) {
      if (getMonthNumber(event.date) !== parseInt(filters.month, 10)) return false
    }
...
export const sortEvents = (events: BarrelRace[], sortBy: SortOption): BarrelRace[] => {
  return [...events].sort((a, b) => {
    switch (sortBy) {
      ...
      case 'added-money-desc':  return b.addedMoney - a.addedMoney
      case 'entry-fee-asc':     return a.entryFee - b.entryFee
