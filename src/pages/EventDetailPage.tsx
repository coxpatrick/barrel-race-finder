...
export default function EventDetailPage() {
  ...
  // Similar events: same state, exclude current, max 3
  const similar = getAllEvents()
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="card overflow-hidden">
              ...
              <div className="p-5 md:p-7">
                <h1 className="font-display text-2xl md:text-3xl lg:text-4xl font-700
                               text-charcoal leading-tight mb-5 text-balance">
                  {event.name}
                </h1>

                {/* Key facts grid — 2 col on mobile, 2 col on desktop */}
                ...
                {/* Divisions as color badges */}
                {event.classes && event.classes.length > 0 && (
                  <div className="mb-5">
                    <p className="font-sans text-xs font-700 text-dust-500 uppercase
                                  tracking-wider mb-2">
                      Divisions
                    </p>
                ...
                {event.notes && (
                  <div className="bg-saddle-50 border border-saddle-100 rounded-xl p-4 md:p-5">
                    <p className="font-sans text-xs font-700 text-saddle-700 uppercase
                                  tracking-wider mb-2">
                      Event Notes
                    </p>
            ...
            <div className="card p-5 md:p-6">
              <h2 className="font-display text-lg font-700 text-charcoal mb-4 flex items-center gap-2">
                <MapPin className="w-5 h-5 text-saddle-500" />
                Venue &amp; Directions
              </h2>

              {/* Map placeholder — swap for real <iframe> when Google Maps API is ready */}
              {/* TODO: Replace with:
                  <iframe
                  ...
                  />
              */}
              <div className="w-full h-44 md:h-52 bg-dust-100 rounded-xl border border-dust-200
                              flex flex-col items-center justify-center gap-3 text-center p-6 mb-4">
                ...
                className="w-full flex items-center justify-center gap-2 py-3 bg-white
                           border-2 border-saddle-300 hover:border-saddle-500 text-saddle-700
                           font-sans font-600 text-sm rounded-xl transition-colors"
              >
          ...
          <div className="space-y-4">
            ...
            <div className="card p-5 space-y-3">
              <p className="font-sans text-xs font-700 text-dust-400 uppercase tracking-wider">
                Event Links
              </p>
              ...
              {event.facebookUrl && (
                <a
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3
                             ...
                             font-600 text-sm rounded-xl transition-colors shadow-sm"
                >
              ...
              {!event.facebookUrl && !event.websiteUrl && (
                <div className="flex items-center gap-2 text-dust-400 text-sm font-sans py-2">
                  <AlertCircle className="w-4 h-4 flex-shrink-0" />
                  No external links provided
                </div>
)}

              {/* TODO: Save to Favorites — add when user auth is implemented */}
            </div>

            {/* Contact card */}
            {(event.contactName || event.contactEmail || event.contactPhone) && (
              <div className="card p-5">
                <p className="font-sans text-xs font-700 text-dust-400 uppercase tracking-wider mb-4">
                  Contact
                </p>
        ...
        {similar.length > 0 && (
          <section className="mt-14 md:mt-16">
            <div className="flex items-end justify-between mb-6">
              <div>
                <h2 className="font-display text-2xl md:text-3xl font-700 text-charcoal">
              ...
              <Link
                ...
                className="hidden sm:flex items-center gap-1 font-sans text-sm font-600
  ...
  return (
    <div className="flex items-start gap-3 p-4 bg-saddle-50 rounded-xl border border-saddle-100">
      ...
      <div className="min-w-0">
        <p className="font-sans text-xs text-saddle-600 font-700 uppercase tracking-wide mb-0.5">
          {label}
        </p>
        <p className="font-sans text-sm font-600 text-charcoal leading-snug">
          {value}
        </p>
        {sub && (
